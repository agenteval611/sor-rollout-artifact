...
class ArtifactSemanticsTest(unittest.TestCase):
    def test_policy_relationships(self):
        stable = rules("policy_stable.json")
        required = rules("policy_required.json")
        unsafe = rules("policy_candidate_unsafe.json")
        safe = rules("policy_candidate_safe.json")
        ...
        self.assertEqual(safe, required)
        self.assertEqual(len(stable - unsafe), 3)
        self.assertGreater(len(required - unsafe), 0)
    ...
    def test_candidate_alias_is_documented_unsafe_candidate(self):
        self.assertEqual(rules("policy_candidate.json"), rules("policy_candidate_unsafe.json"))
