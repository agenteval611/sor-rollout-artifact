#!/usr/bin/env python3
"""Generate scale- and threshold-sensitivity results for the SOR artifact.
...
def small_inputs(seed: int, requests: int):
    ...
    unsafe = rules(load_json(ROOT / "datasets/examples/policy_candidate_unsafe.json"))
    safe = rules(load_json(ROOT / "datasets/examples/policy_candidate_safe.json"))
...
def evaluate_scale(scale: dict, seed: int):
    ...
    # for a withheld unsafe candidate and for a correctly constructed safe candidate.
    ...
    if seed == CONFIG["seeds"][0]:
        ...
        (policy_dir / "policy_candidate_unsafe.json").write_text(json.dumps(as_policy(unsafe_l, f"{scale['key']}-unsafe", "Generated unsafe candidate"), indent=2) + "\n")
        (policy_dir / "policy_candidate_safe.json").write_text(json.dumps(as_policy(safe_l, f"{scale['key']}-safe", "Generated safe candidate"), indent=2) + "\n")
    ...
    return {
        ...
        "Unsafe Proposed Reduction %": f"{pct(stable, unsafe):.1f}",
        "Unsafe Observation Mismatches": unsafe_m,
        "Unsafe Observation Mismatch Rate": f"{unsafe_rate:.6f}",
        "Unsafe Required Would-Deny": unsafe_wd,
        "Unsafe Full SOR Action": "Promoted" if unsafe_pass else "Promotion withheld",
        "Unsafe Legitimate Denials": unsafe_denials,
        "Safe Proposed Reduction %": f"{pct(stable, safe):.1f}",
        "Safe Observation Mismatches": safe_m,
        "Safe Observation Mismatch Rate": f"{safe_rate:.6f}",
        "Safe Required Would-Deny": safe_wd,
        "Safe Full SOR Action": "Promoted" if safe_pass else "Promotion withheld",
        "Safe Legitimate Denials": safe_denials,
        "Raw Event File": str(raw_path.relative_to(ROOT)),
    }


def threshold_rows():
    meta = load_json(ART / "fps_experiment_metadata.json")
    rows = []
    for threshold in CONFIG["threshold_values"]:
        for key, label in [("unsafe", "Unsafe candidate"), ("safe", "Safe candidate")]:
            ...
            rows.append({
                ...
                "Legitimate Denials": 0 if not passed or key == "safe" else "not evaluated",
            })
    return rows
...
def summary_rows(rows: list[dict]):
    ...
    for label in sorted(by_scale, key=lambda x: scale_order[x]):
        ...
        unsafe_rates = [float(r["Unsafe Observation Mismatch Rate"]) for r in group]
        safe_rates = [float(r["Safe Observation Mismatch Rate"]) for r in group]
        out.append({
            "Scale": label,
            ...
            "Seeds": len(group),
            "Unsafe Observation Mismatch Rate Mean": f"{mean(unsafe_rates):.6f}",
            "Unsafe Observation Mismatch Rate Range": f"{min(unsafe_rates):.6f}--{max(unsafe_rates):.6f}",
            "Unsafe Full SOR Actions": "; ".join(sorted(set(r["Unsafe Full SOR Action"] for r in group))),
            "Unsafe Legitimate Denials Total": sum(int(r["Unsafe Legitimate Denials"]) for r in group),
            "Safe Observation Mismatch Rate Mean": f"{mean(safe_rates):.6f}",
            "Safe Observation Mismatch Rate Range": f"{min(safe_rates):.6f}--{max(safe_rates):.6f}",
            "Safe Full SOR Actions": "; ".join(sorted(set(r["Safe Full SOR Action"] for r in group))),
            "Safe Legitimate Denials Total": sum(int(r["Safe Legitimate Denials"]) for r in group),
        })
    return out


def write_latex(summary: list[dict], thresholds: list[dict]):
    scale_lines = []
    for r in summary:
        scale_lines.append(
            f"{r['Scale']} & {r['Identities']:,} & {r['Stable Entitlements']:,} & {r['Required Entitlements']:,} & "
            f"{r['Requests per Seed']:,} $\\times$ {r['Seeds']} & {r['Unsafe Observation Mismatch Rate Mean']} & "
            f"{r['Unsafe Full SOR Actions']} & {r['Safe Observation Mismatch Rate Mean']} & {r['Safe Full SOR Actions']} \\\\" 
        )
    threshold_map = defaultdict(dict)
    for r in thresholds:
        threshold_map[r["Mismatch Threshold"]][r["Candidate"]] = r
    threshold_lines = []
    for t in sorted(threshold_map, key=float):
        u = threshold_map[t]["Unsafe candidate"]
        s = threshold_map[t]["Safe candidate"]
        threshold_lines.append(
            f"{t} & {u['Full SOR Action']} & {s['Full SOR Action']} & "
            f"{u['Required Would-Deny']} & {s['Required Would-Deny']} \\\\" 
        )
    text = r"""% Auto-generated by scripts/run_sensitivity_experiments.py
...
\midrule
""" + "\n".join(threshold_lines) + r"""
\bottomrule
\end{tabular}
\end{table}
"""
    (ROOT / "paper/sensitivity_tables.tex").write_text(text, encoding="utf-8")
...
def main():
    ...
    write_csv(ART / "scale_sensitivity.csv", rows)
    write_csv(ART / "scale_sensitivity_summary.csv", summary)
    write_csv(ART / "threshold_sensitivity.csv", thresholds)
    metadata = {
        **CONFIG,
        "generated_outputs": {
            "per_seed": "artifact_results/scale_sensitivity.csv",
            "summary": "artifact_results/scale_sensitivity_summary.csv",
            "thresholds": "artifact_results/threshold_sensitivity.csv",
            "raw_events": "results/raw/scale_sensitivity/*.csv",
            "generated_policies": "results/generated_scale_policies/*/*.json",
            "latex": "paper/sensitivity_tables.tex",
        },
    }
    (ART / "sensitivity_experiment_metadata.json").write_text(json.dumps(metadata, indent=2) + "\n")
    write_latex(summary, thresholds)
