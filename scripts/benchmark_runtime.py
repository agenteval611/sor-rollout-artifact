...
ROOT=Path(__file__).resolve().parents[1]; ART=ROOT/'artifact_results'; ART.mkdir(exist_ok=True)
N=50_000
...
with (ART/'runtime_benchmark.csv').open('w',newline='') as f:
    w=csv.DictWriter(f,fieldnames=rows[0]); w.writeheader(); w.writerows(rows)
(ART/'runtime_benchmark.json').write_text(json.dumps({'request_count':N,'single_run':True,'host_specific':True,'rollback_predicate_result':rollback,'metrics':rows},indent=2))
