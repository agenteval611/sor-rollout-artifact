#!/usr/bin/env python3
"""Generate FPS paper results from deterministic request-level replay.
...
are evaluated over the same replay:
  * unsafe: removes permissions exercised by legitimate requests;
  * safe: removes only an entitlement excluded from the required-policy oracle.

All aggregate CSV rows and LaTeX tables are derived from request-level events.
No manuscript constants are used.
"""
...
def write_latex(comparison: list[dict], prefix: list[dict]) -> None:
    comparison_lines = [
        f"{r['Candidate']} & {r['Approach']} & {r['Proposed Reduction %']} & {r['Enforced Reduction %']} & "
        f"{r['Legitimate Denials']} & {sci(r['LDR'])} & {r['Affected IDs']} & {int(r['Mismatches']):,} & "
        f"{r['Safety Action']} \\\\" for r in comparison
    ]
    prefix_lines = [
        f"{r['Candidate']} & {r['Replay Prefix']} & {float(r['Agreement S']):.3f} & "
        f"{int(r['Mismatches']):,} & {int(r['Candidate Would-Deny']):,} & {sci(r['Candidate LDR'])} & "
        f"{r['Candidate-Affected IDs']} & {int(r['Request Count']):,} \\\\" for r in prefix
    ]
    text = r"""% Auto-generated by scripts/run_fps_experiments.py
...
\end{table*}
"""
    (PAPER / "fps_tables.tex").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    candidates = [
        Candidate("unsafe", "Unsafe candidate", ROOT / "datasets/examples/policy_candidate_unsafe.json"),
        Candidate("safe", "Safe candidate", ROOT / "datasets/examples/policy_candidate_safe.json"),
    ]
    ...
    write_csv(ART / "baseline_comparison.csv", comparison_rows)
    write_csv(ART / "replay_prefix_sensitivity.csv", prefix_results)
    write_raw(RAW / "fps_replay_events.csv", raw_rows)
    write_latex(comparison_rows, prefix_results)

    obsolete = ART / "observation_window_sensitivity.csv"
    if obsolete.exists():
        obsolete.unlink()
    ...
    (ART / "fps_experiment_metadata.json").write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")
